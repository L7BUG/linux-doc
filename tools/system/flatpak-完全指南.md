...
### 1.4 deb / pacman / Snap / Flatpak 全景对比

这四者常被拿到一起比"谁更好"，但它们其实分属**两个不同层次**——放在同一维度上直接比较是一种错位。理解这个分层，选型就不再纠结。

```
┌────────────────────────────────────────────────┐
│  应用分发层（单个桌面应用：自带依赖 + 沙盒隔离）    │
│      Snap            Flatpak                     │
├────────────────────────────────────────────────┤
│  系统包管理层（管理整个系统：共享系统库）           │
│      deb / apt       pacman                      │
...
- **系统包管理器（`deb`/apt、pacman）**：负责构建和维护**整个操作系统**——内核、驱动、系统库、CLI 到桌面环境。所有软件**共享同一套系统库**，因此磁盘占用小、启动快；代价是一个底层库升级可能牵动全局，且软件版本受发行版节奏约束。
- **通用应用分发（Snap、Flatpak）**：只负责**单个桌面应用**，把依赖一起打包、跑在沙盒里，与系统解耦。好处是跨发行版、安全隔离；代价是占空间、启动略慢。

#### 核心对比

| 维度 | deb (`apt`) | pacman | Snap | Flatpak |
|------|-------------|--------|------|---------|
| **定位** | 系统包管理 | 系统包管理 | 应用分发 | 应用分发 |
| **主导方** | Debian / Ubuntu | Arch Linux | Canonical | freedesktop 社区 |
| **管理对象** | 整个系统 | 整个系统 | 单个桌面应用 | 单个桌面应用 |
| **依赖处理** | 共享系统库 | 共享系统库 | 自带全部依赖 | Runtime + 应用 |
| **沙盒隔离** | ❌ 无 | ❌ 无 | ✅ AppArmor | ✅ Bubblewrap |
| **更新模型** | 发行版节奏 | 滚动更新 | 强制自动更新 | 可选自动更新 |
| **服务端开放性** | ✅ 可自建源 | ✅ 可自建源 | 🔴 闭源独家 | ✅ 可自建仓库 |
| **跨发行版** | ❌ 仅 Debian 系 | ❌ 仅 Arch 系 | ✅ | ✅ |
| **磁盘效率** | ✅ 高（共享库） | ✅ 高（共享库） | ⚠️ 低（各自打包） | ⚠️ 中（共享 Runtime） |
| **回滚能力** | ⚠️ 有限 | ✅ 本地缓存降级 | ✅ 保留旧 revision | 🔴 困难 |
| **典型对象** | 系统库、服务、CLI | 系统库、服务、CLI、AUR | Ubuntu 桌面应用 | 跨发行版桌面应用 |

#### 三个最关键的差异

**1. 共享系统库 vs 自带依赖。** deb/pacman 让所有软件链接同一套系统库——省空间、启动快，但版本被发行版"锁死"，且升级有连锁风险。Snap/Flatpak 各自打包依赖——彼此隔离、跨发行版通用，但重复占用磁盘、启动更慢。

**2. 服务端开放性——最被低估的差异。** pacman、apt、Flatpak 的仓库任何人都能自建、自架镜像；而 **Snap Store 的服务端是闭源的，由 Canonical 独家运营，无法搭建第三方 store**。这正是 Snap 长期争议的核心，也是它与 Flatpak 最本质的区别。

**3. pacman 的滚动 + AUR。** pacman 采用滚动更新（永远最新），配合 AUR 用源码构建，可覆盖 60,000+ 软件——广度和新鲜度远超任何沙盒生态。

#### 选型建议

| 你的需求 | 推荐方案 |
|----------|----------|
| 系统底层 / CLI 工具 / 开发工具链 | **pacman**（Arch）或 **apt**（Debian 系） |
| 攻击面大的桌面应用（浏览器、通讯） | **Flatpak** |
| 需要跨发行版、又能自建仓库 | **Flatpak** |
| 只用 Ubuntu、图省事跟随官方推荐 | Snap（但需接受闭源与强制更新） |
| 追求最新版本 + 海量软件 | **pacman + AUR** |

> **一句话：** `deb`/pacman 负责"操作系统本身"，Snap/Flatpak 负责"桌面应用"，二者互补而非竞争。在 Arch 上——系统层用 pacman，桌面应用优先 Flatpak，Snap 通常无需引入。

### 1.5 什么该放 Flatpak，什么不该放
...
前面 1.5 节已详细展开——IDE、终端、数据库管理工具等在沙箱中严重受限。
